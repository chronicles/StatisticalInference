...
p-values for both equal and unequal variance t test > 5%. We cannot deny the Null hypothesis. So it is not clear that the difference exists between OJ and VC groups.
...
dose.result
```
Above table shows that dosages impacts tooth growth. 

1. p-values are very small.

2. Confidence interval donot contain 0

So, we can deny the null hypothesis and state that higher dosage tends to result in higher tooth length
...
While we cannot say that orange juice and vitamin C have obvious different impact on tooth growth
